...
type GlobalBuffers = {
	__deloop_consoleErrors?: string[];
	__deloop_networkErrors?: string[];
	__deloop_errorCapture?: boolean;
	__deloop_networkCapture?: boolean;
};

const g = globalThis as unknown as GlobalBuffers;

const consoleErrors: string[] = (g.__deloop_consoleErrors ??= []);
const networkErrors: string[] = (g.__deloop_networkErrors ??= []);

// ─── Console error capture ──────────────────────────────────────────────────

try {
	if (!g.__deloop_errorCapture) {
...
try {
	if (!g.__deloop_networkCapture && typeof window !== "undefined") {
		...
		if (XHR) {
			...
			XHR.prototype.open = function patchedOpen(
				this: XMLHttpRequest & { __deloop_method?: string; __deloop_url?: string },
				method: string,
				url: string | URL,
				...rest: unknown[]
			) {
				this.__deloop_method = method.toUpperCase();
				this.__deloop_url = typeof url === "string" ? url : url.href;
			...
			XHR.prototype.send = function patchedSend(
				...
				const method = this.__deloop_method ?? "GET";
				const url = this.__deloop_url ?? "";
		...
		g.__deloop_networkCapture = true;
	}
} catch {}
