...
export async function createDevbarServer(): Promise<{
	app: Hono;
	/** Null when the `cloud` flag is off — there is nothing to sign in to. */
	auth: ReturnType<typeof createAuth> | null;
	/** Null when the `cloud` flag is off — no route touches the database. */
	db: Awaited<ReturnType<typeof getDb>> | null;
}> {
	const app = new Hono();
	const flags = getServerFlags();

	// The hosted SaaS is flagged off by default, and with it every route that
	// needs a database. Only open the connection when something will use it, so
	// a self-hosted server never dials Postgres.
	const db = flags.cloud ? await getDb() : null;
	const auth = db ? createAuth(db) : null;
	...
	// Better Auth routes — sign in, sign up, sessions, OAuth callbacks.
	if (auth) {
	...
	// Dashboard API — every route below belongs to the hosted product and is
	// mounted only when `cloud` is on.
	if (auth) {
		...
		// MCP server (API key auth, no session required) — reads the same cloud
		// reports, so it goes with the rest of the hosted API.
		app.route("", createMcpRoutes());
	}

	// WebSocket collaboration — works without accounts (anonymous peers, or an
	// HMAC token minted by the host app), so it is not part of the cloud flag.
	await addWebSocketRoute(app, auth ?? undefined);
