...
import type { DevbarDb } from "./db";
import { DB_DRIVER, pgSchema, sqliteSchema } from "./db";

export function createAuth(db: DevbarDb): Auth {
	...
	const auth = betterAuth({
		...
		trustedOrigins: process.env.DEVBAR_TRUSTED_ORIGINS?.split(",").map((s) => s.trim()) ?? ["*"],
	}) as unknown as Auth;
	authRef = auth;
	return auth;
}

export type DevbarAuth = ReturnType<typeof createAuth>;
