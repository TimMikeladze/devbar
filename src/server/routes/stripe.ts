...
function getSubscriptionTable() {
	return DB_DRIVER === "sqlite" ? sqliteSchema.subscriptions : (pgSchema.subscriptions as any);
}

const stripeRoutes: Hono<{ Variables: { user: AuthUser } }> = new Hono<{
...
stripeRoutes.post("/checkout", async (c) => {
	...
	const allowedPriceIds = [
		...
		process.env.STRIPE_ORG_PRICE_ID,
	].filter(Boolean);
	...
	const [existing] = await db.select().from(table).where(eq(table.organizationId, orgId));
...
stripeRoutes.post("/portal", async (c) => {
	...
	const [sub] = await db.select().from(table).where(eq(table.organizationId, orgId));
