...
type CommentsTable = PgTableWithColumns<{
	name: "devbar_comments";
	schema: undefined;
	columns: {
		id: PgColumn<
			{
				name: "id";
				tableName: "devbar_comments";
				dataType: "string";
				columnType: "PgText";
				data: string;
				driverParam: string;
...
export const reports: ReportsTable = pgTable(
	...
	(table) => [
		index("idx_devbar_reports_org_id").on(table.organizationId),
		index("idx_devbar_reports_user_id").on(table.userId),
		index("idx_devbar_reports_url").on(table.url),
		index("idx_devbar_reports_created_at").on(table.createdAt),
	],
) as ReportsTable;
...
export const comments: CommentsTable = pgTable(
	...
	(table) => [index("idx_devbar_comments_report_id").on(table.reportId)],
) as CommentsTable;
...
export const subscriptions: any = pgTable(
	...
	(table) => [
		index("idx_devbar_subscriptions_org_id").on(table.organizationId),
		index("idx_devbar_subscriptions_stripe_customer").on(table.stripeCustomerId),
	],
);
...
export const apiKeys: any = pgTable(
	...
	(table) => [index("idx_devbar_api_keys_org_id").on(table.organizationId)],
);
