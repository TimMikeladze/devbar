...
export async function addWebSocketRoute(app: Hono, auth?: DevbarAuth): Promise<void> {
	// `hono/bun` touches Bun globals at import time, so it throws on Node (Vercel).
	// WebSockets are served by the standalone ws-server there, not this route.
	...
	app.get(
		...
		async (c, next) => {
			...
			const flyInstances = process.env.DEVBAR_FLY_INSTANCES?.split(",")
				.map((s) => s.trim())
				.filter(Boolean);
