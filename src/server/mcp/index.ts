...
import type { DevbarPayload, Annotation } from "@/session/types";
...
const API_KEY_HEADER = "x-devbar-api-key";
 ...
 */
function parsePayload(raw: unknown): DevbarPayload | null {
	try {
		const parsed = typeof raw === "string" ? JSON.parse(raw) : raw;
		if (!parsed || typeof parsed !== "object") return null;
		return parsed as DevbarPayload;
	} catch {
		return null;
	}
}
...
function createMcpServerInstance(ctx: McpContext): McpServer {
	...
	server.registerTool(
		"get_annotations",
		{
			...
			description:
				"Get annotations from a report, optionally filtered by type (element, drawing, screenshot, marker, recording).",
			inputSchema: {
				report_id: z.string().describe("Report ID"),
				type: z
					.enum(["element", "drawing", "screenshot", "marker", "recording"])
					.optional()
