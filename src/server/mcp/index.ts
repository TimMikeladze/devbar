...
async function fetchReportScoped(reportId: string, orgId: string | null) {
	...
	const [report] = await db.select().from(reports).where(and(...conditions));
	return report ?? null;
}
...
function createMcpServerInstance(ctx: McpContext, labelScanLimit: number): McpServer {
	...
	server.registerTool("list_reports", {
		...
		inputSchema: {
			...
			label: z.string().optional().describe("Filter by report label (stored in payload JSON, filtered in-memory)"),
		...
		if (args.since) {
			...
			if (!d) {
				return {
					content: [{ type: "text" as const, text: "Invalid 'since' date. Use ISO 8601 format (e.g. 2024-01-15T00:00:00Z)." }],
					isError: true,
				};
			}
		...
		if (args.until) {
			...
			if (!d) {
				return {
					content: [{ type: "text" as const, text: "Invalid 'until' date. Use ISO 8601 format (e.g. 2024-01-15T00:00:00Z)." }],
					isError: true,
				};
			}
		...
		return {
			content: [{
				type: "text" as const,
				text: JSON.stringify({ reports: items, total, limit: args.limit, offset: args.offset }, null, 2),
			}],
		};
	});
	...
	server.registerTool("get_report", {
		title: "Get Report",
		description: "Get a full bug report by ID, including all annotations, page info, console errors, and the generated prompt.",
		inputSchema: {
	...
	server.registerTool("search_reports", {
		...
		description: "Search reports by URL or title using substring matching. Does not search inside payload content — use get_report or get_annotations to inspect specific report details.",
		inputSchema: {
	...
	server.registerTool("get_annotations", {
		...
		description: "Get annotations from a report, optionally filtered by type (element, drawing, text, screenshot, marker, recording).",
		inputSchema: {
			report_id: z.string().describe("Report ID"),
			type: z.enum(["element", "drawing", "text", "screenshot", "marker", "recording"]).optional().describe("Filter by annotation type"),
		},
	}, async (args) => {
		const report = await fetchReportScoped(args.report_id, ctx.organizationId);
		if (!report) return REPORT_NOT_FOUND_ERROR;
	...
	// ── get_element_data ──────────────────────
	server.registerTool("get_element_data", {
		title: "Get Element Data",
		description: "Get deep element details from a specific annotation: selectors, computed styles, accessibility, React component context, bounding rect, form state, and more.",
		inputSchema: {
			report_id: z.string().describe("Report ID"),
			annotation_index: z.number().min(0).describe("Zero-based index of the annotation within the report"),
		},
	}, async (args) => {
		const report = await fetchReportScoped(args.report_id, ctx.organizationId);
		if (!report) return REPORT_NOT_FOUND_ERROR;
	...
	server.registerTool("get_prompt", {
		title: "Get Prompt",
		description: "Get the generated markdown prompt from a bug report. This is the LLM-ready summary of all annotations, page context, and console errors.",
		inputSchema: {
			report_id: z.string().describe("Report ID"),
		},
	}, async (args) => {
...
export function createMcpRoutes(config?: McpConfig): Hono {
	...
	app.post("/mcp", async (c) => {
		...
		if (sessionId) {
			...
			if (session) {
				const authError = await revalidateSession(session, sessionId, c.req.raw, keyCache, sessionStore);
