...
export async function verifyOrgMembership(
  ...
  const memberTable =
    DB_DRIVER === "sqlite" ? sqliteSchema.member : (pgSchema.member as any);
  const [row] = await db
    .select()
    .from(memberTable)
    .where(
...
export function authMiddleware(auth: DeloopAuth): ReturnType<typeof createMiddleware<{ Variables: { user: AuthUser } }>> {
    ...
    async (c, next) => {
      ...
      if (authorName) {
        const allowAnonymous =
          process.env.DELOOP_ALLOW_ANONYMOUS !== "false";
        if (!allowAnonymous)
          return c.json({ error: "Anonymous submissions disabled" }, 401);
        c.set("user", {
          id: null,
          name: authorName,
          email: authorEmail ?? "",
