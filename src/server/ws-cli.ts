...
const start = command({
	name: "start",
	desc: "Start the Devbar WebSocket server",
	options: {
	...
	handler: async (opts) => {
		...
		console.log(`Devbar WS server starting on ${opts.host}:${opts.port}`);
		...
		console.log(`Devbar WS server running at http://${opts.host}:${opts.port}`);
	},
});
