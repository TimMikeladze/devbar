...
export async function createLocalServer(options: LocalServerOptions = {}): Promise<{
	...
	const server = createServer(async (req, res) => {
		const url = req.url ?? "/";
		if (url !== "/health") {
			console.log(`[devbar] ${req.method} ${url}`);
		}
		...
		if (req.method === "POST" && url === "/api/reports") {
			try {
				...
				if (project) {
					...
					if (!projectConfig) {
						console.log(
							`[devbar] report has project="${project}" but no project registered with that slug`,
						);
						console.log(
							`[devbar]   registered projects: ${
								registry
									.list()
									.map((p) => p.slug)
									.join(", ") || "(none)"
						...
						console.log(`[devbar] report for "${project}" saved (auto-dispatch disabled)`);
					} else {
						taskId = dispatcher.enqueue(filePath, project);
					}
				} else {
					console.log("[devbar] report saved without project field — skipping dispatch");
				}
