			...
			<div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-10 lg:gap-12 items-center">
				...
				<div className="hero-stagger text-center lg:text-left">
					...
					<p className="text-dim text-[15px] sm:text-[17px] leading-[1.65] mb-7 max-w-xl mx-auto lg:mx-0">
						devbar is a drop-in inspector for any website. Point at an element, note the change you
					...
					<div className="flex items-center justify-center lg:justify-start gap-3 flex-wrap">
						...
						<a
							href="https://github.com/TimMikeladze/devbar"
							target="_blank"
...
function HowItWorks() {
	...
	const steps = [
		...
		},
		{
			n: "02",
			title: "Capture",
			desc: "devbar reads the DOM and React fiber tree — selectors, styles, components, and source paths.",
...
<script>window.Devbar.init();</script>`,
		},
		{
			...
			lang: "tsx",
			code: `<Devbar server="https://api.yourapp.com/bugs" />
...
function InstallTabs() {
	const [active, setActive] = useState<number>(() => {
		try {
			const saved = localStorage.getItem("devbar-pm");
	...
	const select = (i: number) => {
		setActive(i);
		try {
			localStorage.setItem("devbar-pm", pkgManagers[i]!.label);
		} catch {}
	};
...
function FAQ() {
	const faqs = [
		...
		},
		{
			...
			a: "DevTools is for one developer debugging alone. devbar lets anyone — design, PM, QA — point at a change and hand off the exact context.",
		},
		{
				...
				<div className="grid sm:grid-cols-2 gap-8 sm:gap-10 items-start">
					...
					<div>
						...
						<div className="space-y-3">
							<a
								href="https://github.com/TimMikeladze/devbar/issues"
								target="_blank"
								rel="noopener noreferrer"
...
const footerLinks = [
	{ href: "https://github.com/TimMikeladze/devbar", label: "GitHub", icon: <GithubGlyph /> },
		...
		<footer className="border-t border-border pt-8 mt-4 pb-8">
			<div className="flex flex-wrap items-center gap-1 text-[13px]">
				...
				<div className="ml-auto flex items-center gap-4">
					<span className="text-muted text-xs">&copy; {new Date().getFullYear()} devbar.sh</span>
				</div>
			</div>
		</footer>
	);
