...
**Goal:** Add a dispatch system to the devbar local server that spawns Claude Code CLI processes to handle incoming annotation reports, with per-project config, concurrency control, and both auto and manual dispatch.
...
```
src/server/
  registry.ts       # NEW — project registration, in-memory Map + ~/.devbar/projects.json persistence
...
src/toolbar/
  toolbar.tsx        # MODIFY — add project prop to DevbarProps, include in POST body
test/
...
export type LocalServerOptions = {
	...
	resultsDir?: string;
	/** Path to projects.json (default: ~/.devbar/projects.json) */
	projectsFile?: string;
...
# Expected: "server started, project 'devbar' registered"
...
# Verify via API
curl -s -H "Authorization: Bearer $(cat ~/.devbar/token)" http://127.0.0.1:3199/api/projects
...
- [ ] **Step 1: Add `project` to `DevbarProps`**
...
In the `Devbar` function signature (around line 765):
...
sleep 1
TOKEN=$(cat ~/.devbar/token)

# Register another project
cd /tmp && node /Users/tim/workspace/devbar/dist/local/local-cli.js --port 3199 --name tmp-proj --model haiku --effort low
cd /Users/tim/workspace/devbar
...
curl -s -X POST http://127.0.0.1:3199/api/reports \
  ...
  -d '{"payload":{"prompt":"fix the button","annotations":[],"url":"http://localhost:3000","timestamp":1},"url":"http://localhost:3000","title":"Test","project":"devbar"}'
